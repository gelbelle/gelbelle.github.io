...
const getOp = (calculator, ans = 0) => {
    console.log(`toCalc: ${calculator.toCalc}`);
...
const handleOperations = (target, calculator) => {
    ...
    if (calculator.answered) calculator.toCalc.splice(1);
    calculator.answered = false;
    ...
    if (!calculator.answered) {
        if (!calculator.display.value.includes(target) || calculator.lastPressed.classList.contains("ops")) {
            if (calculator.displayChanged) {
                (calculator.display.value === "0") ? updateDisplay("0.", calculator) :
                    updateDisplay(calculator.display.value + ".", calculator)
            } else updateDisplay("0.", calculator)
        }
    } else updateDisplay("0.", calculator);
}
...
//TODO display updating correctly. Need to correct NaN when pressing equals for a second time
    ...
    if (!opClicked(calculator.operators)) {
        if (!calculator.answered) {
            if (calculator.display.value === "0") {
                ...
                updateDisplay(num, calculator);
            } else updateDisplay(calculator.display.value + num, calculator);
        } else {
            calculator.answered = false;
            calculator.prevDisplay = calculator.display.value;
            updateDisplay(num, calculator);
        ...
        if (calculator.display.value == calculator.prevDisplay) {
            ...
            updateDisplay(calculator.display.value + num, calculator);
        } else updateDisplay(num, calculator);
    }
    console.log(`After number ${calculator.toCalc}`);
}
